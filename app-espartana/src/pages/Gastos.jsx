...
import { CreditCard, Minus, ArrowUpRight, CheckCircle2, Loader2, ArrowLeft, Trash2 } from 'lucide-react';
...
const Gastos = () => {
  ...
  const fetchGastos = async () => {
    try {
      ...
      const gastosData = querySnapshot.docs.map(documento => ({
        id: documento.id,
        ...documento.data(),
  ...
  // NUEVA FUNCIÓN TÁCTICA: Eliminar un Gasto
  const handleEliminar = async (id) => {
    const confirmar = window.confirm("¿Estás seguro de cancelar esta baja? El capital volverá a tu munición total.");
    if (confirmar) {
      try {
        await deleteDoc(doc(db, "gastos", id));
        fetchGastos(); // Recargamos la lista después de borrar
      } catch (error) {
      ...
      <section className="bg-white dark:bg-[#111] p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-lg mb-8">
        ...
        <form onSubmit={handleRegistrarGasto} className="space-y-5">
          ...
          <div>
            ...
            <input 
              type="text" placeholder="Ej: Coto, luz, gas..." value={descripcion} onChange={(e) => setDescripcion(e.target.value)}
      ...
      <section>
        ...
        {loading && gastos.length === 0 ? (
          ...
          <div className="space-y-3">
            {gastos.map((gasto) => (
              <div key={gasto.id} className="flex justify-between items-center p-4 bg-white dark:bg-[#111] rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm relative overflow-hidden">
